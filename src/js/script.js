...
function vehicleDisplayYear(vehicle) {
  return vehicle.modelYear || vehicle.model_year || vehicle.year || "Consulte";
}
...
function filteredVehicles() {
  ...
  result.sort((a, b) => sort === "price-asc" ? a.price - b.price : sort === "price-desc" ? b.price - a.price : sort === "year-desc" ? Number(vehicleDisplayYear(b)) - Number(vehicleDisplayYear(a)) : Number(b.featured) - Number(a.featured));
  return result;
}
...
function stockFeedSlideMarkup(vehicle, extraClass = "") {
  ...
  return `<div class="feed-slide ${extraClass}" data-feed-slide>
    ...
    <div class="feed-info">
      ...
      <ul><li>${vehicleDisplayYear(vehicle)}</li><li>${vehicle.mileage ? `${number.format(vehicle.mileage)} km` : "Km consulte"}</li><li>${escapeHTML(vehicle.transmission || "Câmbio consulte")}</li></ul>
